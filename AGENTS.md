...
- **vibox never retags a track, and touches a music folder only in the ways the user asked for:** a rename typed in the edit buffer, and, with `:set danger` on, a move, copy, delete or `:mkdir`. Everything else about a library is read only. The README states this at the top, so a feature that widens it without widening that paragraph is a lie in the documentation as well as a bug.
- **`danger` is never persisted.** It is left out of the session state and out of `:mkrc`, so the only ways to have it on are typing `:set danger` in that session or putting it in `viboxrc` by hand. It once leaked into the saved state and came back silently every launch, which is the one failure this option must not have.
